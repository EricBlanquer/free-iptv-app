...
var DevRemoteSync = {
    ...
    init: function() {
        ...
        window.log('DevRemoteSync.init: enabled=' + this.isDevDevice + ' deviceId=' + window.deviceId);
    },
            ...
            xhr.onload = function() {
                if (xhr.status === 200) {
                    try {
                        ...
                        window.log('DevRemoteSync.load: success, keys=' + Object.keys(data).join(','));
                        resolve(data);
                    }
                    catch (ex) {
                        window.log('DevRemoteSync.load: parse error ' + ex.message);
                        resolve(null);
                    }
                }
                else {
                    window.log('DevRemoteSync.load: status=' + xhr.status);
                    resolve(null);
                }
            };
            xhr.onerror = function() {
                window.log('DevRemoteSync.load: network error');
                resolve(null);
            };
            xhr.ontimeout = function() {
                window.log('DevRemoteSync.load: timeout');
                resolve(null);
            };
            xhr.send();
        });
    ...
    doSave: function(data) {
        ...
        try {
            ...
            window.log('DevRemoteSync.save: sent');
        }
        catch (ex) {
            window.log('DevRemoteSync.save: error ' + ex.message);
        }
    },
    ...
    applyData: function(app, remoteData) {
        ...
        if (remoteTime > localSettingsTime || localSettingsTime === 0) {
            window.log('DevRemoteSync.applyData: applying remote data (remote=' + remoteTime + ' local=' + localSettingsTime + ')');
            ...
            window.log('DevRemoteSync.applyData: local is newer, not applying');
        }
        return applied;
    },
        ...
        return new Promise(function(resolve) {
            ...
            xhr.onload = function() {
                if (xhr.status === 200 && xhr.responseText) {
                    try {
                        var data = JSON.parse(xhr.responseText);
                        window.log('DevRemoteSync.loadProviderCache: success for ' + playlistId);
                        resolve(data);
                    }
                    catch (ex) {
                        window.log('DevRemoteSync.loadProviderCache: parse error ' + ex.message);
                        resolve(null);
                    }
                }
                else {
                    window.log('DevRemoteSync.loadProviderCache: status=' + xhr.status);
                    resolve(null);
                }
            };
            xhr.onerror = function() {
                window.log('DevRemoteSync.loadProviderCache: network error');
                resolve(null);
            };
            xhr.ontimeout = function() {
                window.log('DevRemoteSync.loadProviderCache: timeout');
                resolve(null);
            };
            xhr.send();
        });
    ...
    doSaveProviderCache: function(playlistId, data) {
        ...
        try {
            ...
            xhr.onload = function() {
                if (xhr.status === 200) {
                    window.log('DevRemoteSync.saveProviderCache: saved for ' + playlistId + ' response=' + xhr.responseText);
                }
                else {
                    window.log('DevRemoteSync.saveProviderCache: server error ' + xhr.status + ' for ' + playlistId + ' response=' + xhr.responseText);
                }
            };
            xhr.onerror = function() {
                window.log('DevRemoteSync.saveProviderCache: network error for ' + playlistId);
            };
            xhr.send(JSON.stringify({
                device: this.devDeviceId,
                playlist: playlistId,
            ...
            window.log('DevRemoteSync.saveProviderCache: sent for ' + playlistId);
        }
        catch (ex) {
            window.log('DevRemoteSync.saveProviderCache: error ' + ex.message);
        }
    }
};
