...
IPTVApp.prototype.getStreamImage = function(stream) {
    if (!stream) return '';
    if (typeof ChannelLogos !== 'undefined' && !stream.container_extension && !stream.series_id) {
        var logo = this.getChannelLogo(stream);
        if (logo) return logo;
    }
    return stream.stream_icon || stream.cover || '';
};

IPTVApp.prototype.getChannelLogo = function(stream) {
    if (typeof ChannelLogos === 'undefined') return '';
    var raw = (stream.name || stream.title || '');
    var pipeIdx = raw.lastIndexOf('|');
    var name = (pipeIdx !== -1 ? raw.substring(pipeIdx + 1) : raw).toUpperCase()
        .replace(/^[A-Z]{2,3}[\-:]\s*/, '')
        .replace(/\s*(FHD|UHD|HD|SD|4K|8K|HEVC|H265|H\.265)\s*/g, ' ')
        .trim();
    if (ChannelLogos[name]) return ChannelLogos[name];
    if (!ChannelLogos._sortedKeys) {
        ChannelLogos._sortedKeys = Object.keys(ChannelLogos).filter(function(k) { return k !== '_sortedKeys'; })
            .sort(function(a, b) { return b.length - a.length; });
    }
    var keys = ChannelLogos._sortedKeys;
    for (var i = 0; i < keys.length; i++) {
        if (name.length > keys[i].length && name.indexOf(keys[i] + ' ') === 0) {
            return ChannelLogos[keys[i]];
        }
    }
    return '';
};
