...
IPTVApp.prototype.getDetailsZones = function(focusables) {
    var zones = {};
    var zoneOrder = ['favorite', 'versions', 'actions', 'seasons', 'episodes', 'director', 'cast'];
    for (var i = 0; i < focusables.length; i++) {
...
IPTVApp.prototype.applyMarqueeToFocusedElement = function(el) {
    ...
    if (el.classList.contains('category-item')) {
        ...
        var containerWidth = el.clientWidth - 20;
...
IPTVApp.prototype.updateFocus = function() {
    ...
    else if (focusables.length === 0 && !this.restoringFocus) {
...
IPTVApp.prototype.scrollHorizontalGridToElement = function(el) {
    var horizontalGrid = el.closest('#details-cast-grid, #details-director-grid, #actor-filmography-grid');
