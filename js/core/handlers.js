...
IPTVApp.prototype.selectHandlers = {
    ...
    'premium-modal': function(current) {
        Premium.hidePremiumOverlay();
        if (current.id === 'premium-contribute-btn') {
            this.openSection('settings');
            var self = this;
            setTimeout(function() {
                var licenseInput = document.getElementById('setting-licenseCode');
                if (licenseInput) licenseInput.scrollIntoView({ block: 'center' });
            }, 300);
        }
    },
    ...
    details: function(current) {
        if (current.id === 'details-description') {
            if (!TMDB.isEnabled() && current.textContent === I18n.t('details.configureTMDB')) {
                this.showSettings();
                this.toggleCollapsible('apis');
                return;
            }
...
IPTVApp.prototype.backHandlers = {
    ...
    'focusArea:premium-modal': function() {
        Premium.hidePremiumOverlay();
    },
