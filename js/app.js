...
class IPTVApp {
    ...
    constructor() {
        ...
        this.selectedCategoryBySection = {};
        this.focusIndex = 0;
    ...
    init() {
        ...
        if (DevRemoteSync.isDevDevice) {
            ...
            DevRemoteSync.load().then(function(remoteData) {
                if (remoteData && DevRemoteSync.applyData(self, remoteData)) {
                    window.log('SYNC', 'Remote data applied, refreshing UI');
        ...
        this.player.setProxyUrl(this.settings.proxyUrl);
            ...
            this.loadProviderCache(playlist.id).then(function(providerCache) {
                ...
                if (hasValidCache) {
                    ...
                    self.providerCacheInfo = {
                        ...
                        timestamp: providerCache._cacheTimestamp || Date.now()
                    };
                    ...
                    document.getElementById('home-grid').style.visibility = '';
                    self.showLoading(false);
                    // Trigger background refresh if cache is stale (delayed to not block UI)
                    ...
                    self.api.authenticate().then(function() {
                        ...
                        self.api.preloadCache(function(step, total, name) {
                            ...
                            if (step === -1) {
                                ...
                                self.showToast('Timeout ' + loadingText, 5000, true);
                            }
                            else if (step === 0) {
        ...
        this.loadProviderCache('merged').then(function(mergedCache) {
            if (mergedCache && mergedCache.vod && mergedCache.vod.categories) {
                ...
                self.providerCacheInfo = {
                    ...
                    timestamp: mergedCache._cacheTimestamp || Date.now()
                };
