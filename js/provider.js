...
class ProviderAPI {
    ...
    async fetchWithRetry(url, retries = this.maxRetries, timeout = this.defaultTimeout) {
        ...
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                window.log('HTTP> ' + ProviderAPI.redactUrl(fetchUrl) + ' (timeout=' + (timeout/1000) + 's)');
                ...
                var logMsg = 'HTTP< ' + response.status + ' ' + ProviderAPI.redactUrl(url);
                if (response.redirected) {
                    logMsg += ' -> ' + ProviderAPI.redactUrl(response.url);
                }
                window.log(logMsg);
                if (!response.ok) {
     ...
     */
    _clearCategoryEntries(cacheObj) {
    ...
    filterCacheByLanguage(matchFn) {
        ...
        if (Array.isArray(this.cache.vodCategories)) {
            ...
            this._clearCategoryEntries(this.cache.vodStreams);
        }
        if (Array.isArray(this.cache.seriesCategories)) {
            ...
            this._clearCategoryEntries(this.cache.series);
        }
        var after = {
            vodCat: this.cache.vodCategories ? this.cache.vodCategories.length : 0,
            vodStreams: this.cache.vodStreams['_all'] ? this.cache.vodStreams['_all'].length : 0,
