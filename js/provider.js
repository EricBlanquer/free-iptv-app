    ...
    async fetchWithRetry(url, retries = this.maxRetries) {
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                ...
                if (window.isDevDevice && window.isDevDevice()) {
                    const cloned = response.clone();
                    const logUrl = url;
                    cloned.text().then(function(body) {
                        var truncated = body.length > 500 ? body.substring(0, 500) + '...' : body;
                        window.log('HTTP body ' + logUrl + ': ' + truncated);
                    }).catch(function() { /* logging only */ });
                }
                ...
                window.log('ERROR', 'HTTP ' + error.message + ' ' + url);
                if (attempt === retries) {
                    throw error;
                }
