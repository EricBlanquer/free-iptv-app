...
IPTVApp.prototype.loadVisibleImages = function(forceFromStart) {
        ...
        if (!imageUrl) {
            ...
            (function(div, url, idx) {
                ...
                var timeoutId = setTimeout(function() {
                    ...
                    window.log('IMG [' + idx + '] TIMEOUT after 30s: ' + url);
                }, 30000);
                img.onload = function() {
                    ...
                    if (duration > 1000) {
                        window.log('IMG [' + idx + '] SLOW ' + duration + 'ms: ' + url);
                    }
                };
                img.onerror = function() {
                    ...
                    div.classList.add('no-image');
                    window.log('IMG [' + idx + '] ERROR ' + duration + 'ms: ' + url);
                };
                img.src = url;
            })(imageDiv, imageUrl, i);
        }
    ...
    if (toLoad > 0 || noUrl > 0) {
        window.log('IMG load: ' + toLoad + ' to load, ' + noUrl + ' no URL, ' + alreadyLoaded + ' cached (range ' + startIdx + '-' + endIdx + ')');
    }
};
        ...
        (function(gridItem, title, tp, tid) {
            self.fetchTMDBCached(title, tp, function(result) {
                ...
                if (result) {
                    ...
                    var runtime = result.runtime || (result.episode_run_time && result.episode_run_time[0]);
                    if (runtime && runtime > 0) {
                        var durationSpan = infoDiv.querySelector('.grid-duration');
                        if (!durationSpan) {
                            durationSpan = document.createElement('span');
                            durationSpan.className = 'grid-duration';
                            infoDiv.appendChild(durationSpan);
                        }
                        var hours = Math.floor(runtime / 60);
                        var mins = runtime % 60;
                        durationSpan.textContent = hours > 0 ? hours + 'h' + (mins > 0 ? mins.toString().padStart(2, '0') : '') : mins + 'min';
                    }
                    // Add genres
...
IPTVApp.prototype.loadCategory = function(section) {
    ...
    if (!this.api) {
        window.log('loadCategory: no API, checking memory data for ' + apiSection);
        ...
        promise = this.loadProviderCache('merged').then(function(providerCache) {
            if (providerCache && providerCache[apiSection] && providerCache[apiSection].categories && providerCache[apiSection].categories.length > 0) {
                window.log('loadCategory: CACHE HIT merged v2 for ' + apiSection);
                return [providerCache[apiSection].categories, providerCache[apiSection].streams || []];
            }
            // No cache, load from APIs
            return self.loadMergedData(apiSection, section).then(function(results) {
                self.cacheProviderData('merged', apiSection, results[0], results[1]);
                return results;
            });
        });
    }
        ...
        promise = Promise.all([
            ...
            self.cacheProviderData(playlistId, 'live', results[0], results[1]);
            return results;
        });
    }
        ...
        promise = Promise.all([
            ...
            var combined = [vodCats.concat(seriesCats), vodStreams.concat(seriesStreams)];
            self.cacheProviderData(playlistId, 'vod', combined[0], combined[1]);
            return combined;
        });
    }
        ...
        promise = Promise.all([
            ...
            self.cacheProviderData(playlistId, 'vod', results[0], results[1]);
            return results;
        });
    }
    else {
        promise = Promise.all([
            ...
            self.cacheProviderData(playlistId, 'series', results[0], results[1]);
            return results;
        });
    }
    promise.then(function(results) {
        ...
        // Filter SD streams: keep only if no HD equivalent exists
        if (self.hideSD) {
            var titleMap = {};
            filteredStreams.forEach(function(s) {
                var cleanTitle = self.cleanTitle(s.name || s.title || '').toLowerCase();
            ...
            if (hiddenCount > 0) window.log('HIDDEN (SD has HD): ' + hiddenCount);
            // Hide SD categories that have no streams left
...
IPTVApp.prototype.loadMergedData = function(apiSection, section) {
    ...
    return Promise.all(promises).then(function(results) {
        ...
        window.log('loadMergedData: ' + allCategories.length + ' categories, ' + allStreams.length + ' streams, playlistIds=' + Object.keys(playlistIds).join(','));
...
IPTVApp.prototype.loadStreams = function(categoryId, options) {
    ...
    // Get real categoryId and playlistId from selected category element
    ...
    if (!this.api || isMergeMode) {
        // For merge mode or no API, use data already loaded in memory
        ...
        var filtered = categoryId ? allStreams.filter(function(s) {
            var matchCategory = String(s.category_id) === String(realCategoryId);
            var matchPlaylist = !categoryPlaylistId || String(s._playlistId) === String(categoryPlaylistId);
    ...
    promise.then(function(streams) {
        // Apply SD filter if needed (keep SD only if no HD exists)
...
IPTVApp.prototype.renderCategories = function(categories, streams) {
    ...
    if (continueCount > 0) {
        ...
        continueItem.className = 'category-item category-continue';
    ...
    if (favoritesCount > 0) {
        ...
        favoritesItem.className = 'category-item category-favorites';
...
IPTVApp.prototype.applyFilters = function() {
    ...
    if (this.hideSD) {
        // Only hide SD if an HD version of the same title exists
        var titleMap = {};
        streams.forEach(function(s) {
            var cleanTitle = self.cleanTitle(s.name || s.title || '').toLowerCase();
            if (!titleMap[cleanTitle]) {
        ...
        streams = streams.filter(function(s) {
            if (!self.isSD(s)) return true;
            var cleanTitle = self.cleanTitle(s.name || s.title || '').toLowerCase();
            // Keep SD only if no HD version exists
            return titleMap[cleanTitle].hd.length === 0;
        });
        var hiddenCount = beforeCount - streams.length;
        if (hiddenCount > 0) window.log('FILTER (SD has HD): ' + hiddenCount);
    }
    if (this.hideSM) {
        streams = streams.filter(function(s) {
            return !self.isSM(s);
    ...
    // Detect duplicate titles for disambiguation
    var titleCounts = {};
    var titleGenreCounts = {};
    streams.forEach(function(s) {
        var cleanTitle = self.cleanTitle(s.name || s.title || '').toLowerCase();
        var genre = (s.genre || '').split(',')[0].trim().toLowerCase();
        var titleGenreKey = cleanTitle + '|' + genre;
        titleCounts[cleanTitle] = (titleCounts[cleanTitle] || 0) + 1;
        titleGenreCounts[titleGenreKey] = (titleGenreCounts[titleGenreKey] || 0) + 1;
    });
    streams.forEach(function(s) {
        var cleanTitle = self.cleanTitle(s.name || s.title || '').toLowerCase();
        var genre = (s.genre || '').split(',')[0].trim().toLowerCase();
        var titleGenreKey = cleanTitle + '|' + genre;
        s._isDuplicate = titleCounts[cleanTitle] > 1;
        s._needsId = titleGenreCounts[titleGenreKey] > 1;
    });
    this.currentStreams = streams;
    this.displayedCount = 0;
    document.getElementById('content-grid').innerHTML = '';
    // Use requestAnimationFrame to not block UI during category navigation
    requestAnimationFrame(function() {
        self.loadMoreItems();
    });
};
...
IPTVApp.prototype.loadMoreItems = function() {
    ...
    for (var i = startIndex; i < endIndex; i++) {
        ...
        var originalTitle = stream.name || stream.title || 'Unknown';
        ...
        if (this.searchTitle && stream.genre) {
            var genreTag = document.createElement('span');
            genreTag.className = 'grid-item-genre';
            genreTag.textContent = ' - ' + this.formatDisplayTitle(stream.genre);
        ...
        // Add disambiguation info for duplicate titles
        if (stream._isDuplicate) {
            var disambigParts = [];
            // Only add genre if it helps differentiate (not same title+genre)
            if (stream.genre && !stream._needsId) {
                var genreName = this.formatDisplayTitle(stream.genre.split(',')[0].trim());
                if (genreName) disambigParts.push(genreName);
            }
            if (stream._playlistId && this.settings.playlists && this.settings.playlists.length > 1) {
                var playlist = this.settings.playlists.find(function(p) { return p.id === stream._playlistId || p.name === stream._playlistId; });
                if (playlist && playlist.name) disambigParts.push(playlist.name);
            }
            // Add stream ID if same title+genre appears multiple times
            if (stream._needsId && stream.stream_id) {
                disambigParts.push('#' + stream.stream_id);
            }
            if (disambigParts.length > 0) {
                var disambigSpan = document.createElement('span');
                disambigSpan.className = 'grid-disambig';
                disambigSpan.textContent = ' - ' + disambigParts.join(' | ');
                info.appendChild(disambigSpan);
            }
        }
...
IPTVApp.prototype.updateContinueCounter = function() {
    ...
    if (continueCount === 0) {
        ...
        continueItem.textContent = I18n.t('home.continue', 'Continue') + ' (' + continueCount + ')';
    } else {
...
IPTVApp.prototype.updateFavoritesCounter = function() {
    ...
    if (favoritesCount === 0) {
        ...
        favoritesItem.textContent = I18n.t('home.favorites', 'Favorites') + ' (' + favoritesCount + ')';
    } else {
        // Create category if it doesn't exist (insert after Continue or at start)
        var container = document.getElementById('category-list');
