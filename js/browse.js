...
IPTVApp.prototype.loadCategory = function(section) {
    ...
    promise.then(function(results) {
        ...
        setTimeout(function() {
            ...
            // Check if we have a saved category for this section
            var categoryKey = (self.settings.activePlaylistId || '') + '_' + section;
            var savedCategory = self.selectedCategoryBySection[categoryKey];
            // Validate saved category exists (special categories or in categories list)
            var specialCategories = ['', 'tnt', 'favorites', 'continue', 'guide'];
            var categoryExists = savedCategory === undefined ||
                specialCategories.indexOf(savedCategory) !== -1 ||
                categories.some(function(c) { return c.category_id === savedCategory || String(c.category_id) === String(savedCategory); });
            if (savedCategory !== undefined && categoryExists) {
                self.loadStreams(savedCategory);
            }
            else {
                // Clear invalid saved category
                if (savedCategory !== undefined && !categoryExists) {
...
IPTVApp.prototype.applyFilters = function() {
    ...
    // Clean up duplicate properties from cache before recalculating
    streams.forEach(function(s) {
        delete s._duplicateVersions;
        delete s._duplicateInfos;
        delete s._duplicateTag;
    ...
    streams.forEach(function(s) {
        var cleanTitle = self.cleanTitle(self.getStreamTitle(s)).toLowerCase();
        var year = self.extractYear(self.getStreamTitle(s));
        var titleYearKey = cleanTitle + '|' + (year || '');
        titleCounts[cleanTitle] = (titleCounts[cleanTitle] || 0) + 1;
        if (!titleGroups[titleYearKey]) titleGroups[titleYearKey] = [];
        titleGroups[titleYearKey].push(s);
    });
    var hideSD = self.hideSD;
    var hide3D = self.hide3D;
    Object.keys(titleGroups).forEach(function(key) {
        var group = titleGroups[key];
        if (group.length > 1) {
            ...
            var hasBetterQuality = false;
            var allWords = group.map(function(s) {
                return (s.name || s.title || '').split(/[\s|:\-()]+/).filter(Boolean);
            });
            var commonWords = allWords[0].filter(function(word) {
                return allWords.every(function(words) {
                    return words.some(function(w) { return w.toLowerCase() === word.toLowerCase(); });
                });
            });
            var duplicateInfos = [];
            var tags = [];
            group.forEach(function(s, idx) {
                s._duplicateNum = idx + 1;
                var originalTitle = self.getStreamTitle(s);
                ...
                if (diff) {
                    s._duplicateTag = diff;
                    if (qualityMatch && Regex.sdQualities.indexOf(qualityMatch[1].toUpperCase()) === -1) hasBetterQuality = true;
                }
                tags.push(diff);
                duplicateInfos.push({ id: s.stream_id || s.series_id, name: s.name || s.title || '', num: idx + 1 });
            });
            var uniqueTags = tags.filter(function(t, i) { return tags.indexOf(t) === i; });
            if (uniqueTags.length > 1 || (uniqueTags.length === 1 && uniqueTags[0] !== '')) {
                group.forEach(function(s) {
                    s._duplicateInfos = duplicateInfos;
                });
                var versions = group.map(function(s) {
                    return {
                ...
                versions.sort(function(a, b) {
                    return qualityScore(b.tag) - qualityScore(a.tag);
                });
    ...
    streams = streams.filter(function(s) { return !s._hiddenDuplicate; });
    streams.forEach(function(s) {
        var cleanTitle = self.cleanTitle(self.getStreamTitle(s)).toLowerCase();
