...
IPTVApp.prototype.loadCategory = function(section) {
    ...
    promise.then(function(results) {
        // Skip preprocessing if already done for this section (by preloadSections or previous visit)
        if (self.data[section] && self.data[section]._dedupGroups) {
            window.log('CACHE', 'loadCategory: reusing preprocessed data for ' + section);
            var cachedData = self.data[section];
            setTimeout(function() {
                self.renderCategories(cachedData.categories, cachedData.streams);
                ...
                var categoryExistsInData = savedCategory !== undefined &&
                    cachedData.categories.some(function(c) { return c.category_id === savedCategory || String(c.category_id) === String(savedCategory); });
                if (categoryExistsInSidebar || categoryExistsInData) {
                    ...
                    var hasTnt = section === 'live' && tntChannels.length > 0 && self.getTntStreamsCount(cachedData.streams, tntChannels) > 0;
                    if (hasTnt) {
                        self.showTntInGrid();
                        self.updateCategorySelection('tnt');
                    }
                    else {
                        var isVodSub = ['sport', 'manga', 'entertainment'].indexOf(section) !== -1 || section.indexOf('custom_') === 0;
                        self.renderGrid(cachedData.streams, isVodSub ? 'vod' : section);
        ...
        var onDone = function() {
            ...
            if (!results._fromCache) {
                ...
                if (cachePlaylistId) {
                    self.cacheProviderData(cachePlaylistId, apiSection, sectionData.categories, sectionData.streams);
                }
            }
            ...
            var categoryExistsInSidebar = savedCategory !== undefined && sidebarContainer &&
                sidebarContainer.querySelector('[data-category-id="' + savedCategory + '"]') !== null;
            var categoryExistsInData = savedCategory !== undefined &&
                sectionData.categories.some(function(c) { return c.category_id === savedCategory || String(c.category_id) === String(savedCategory); });
            if (categoryExistsInSidebar || categoryExistsInData) {
                self.loadStreams(savedCategory);
            }
            else {
                ...
                var tntChannels = I18n.getTntChannels();
                var hasTnt = section === 'live' && tntChannels.length > 0 && self.getTntStreamsCount(sectionData.streams, tntChannels) > 0;
                if (hasTnt) {
                    self.showTntInGrid();
                    self.updateCategorySelection('tnt');
                }
                else {
                    self.renderGrid(sectionData.streams, isVodSubsection ? 'vod' : section);
                    self.updateCategorySelection('');
                }
            }
            self.showLoading(false);
...
IPTVApp.prototype.preloadSections = function() {
    var self = this;
...
IPTVApp.prototype._invalidatePreprocessCache = function() {
    ...
    window.log('CACHE', 'Preprocessing cache invalidated');
};
...
IPTVApp.prototype._preprocessStreams = function(streams, categories, categoryMap, onProgress) {
    ...
    var finalize = function() {
        ...
        if (self.hideSD) {
            var beforeSD = filtered.length;
            filtered = filtered.filter(function(s) {
                return !s._isSD || !titleMap[s._dedupCleanTitle].hd;
            });
            if (beforeSD !== filtered.length) {
                window.log('CACHE', 'HIDDEN (SD has HD): ' + (beforeSD - filtered.length));
            }
            var activeAfterSD = {};
            for (var fi = 0; fi < filtered.length; fi++) activeAfterSD[filtered[fi].category_id] = true;
            categories = categories.filter(function(cat) {
                var name = (cat.category_name || '').toUpperCase();
                if (!name.startsWith('SD|')) return true;
                return activeAfterSD[cat.category_id];
            });
        }
        if (self.hide3D) {
            var before3D = filtered.length;
            filtered = filtered.filter(function(s) { return !s._is3D; });
            if (before3D !== filtered.length) {
                window.log('CACHE', 'HIDDEN (3D): ' + (before3D - filtered.length));
            }
            var activeAfter3D = {};
            for (var fi = 0; fi < filtered.length; fi++) activeAfter3D[filtered[fi].category_id] = true;
            categories = categories.filter(function(cat) {
                var name = (cat.category_name || '').toUpperCase();
                if (name.indexOf('3D') === -1) return true;
                return activeAfter3D[cat.category_id];
            });
        }
        if (needsGenre && needsSDFilter && filtered.length < beforeCount) {
            genreSet = {};
            genreKeyMap = {};
            for (var ri = 0; ri < filtered.length; ri++) {
                var sg = filtered[ri];
                if (sg.genre) {
                    var rdg = sg.genre.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"').replace(/&#39;/g, "'");
                    var rGenres = rdg.split(/[\/,&]/);
                    for (var rgi = 0; rgi < rGenres.length; rgi++) {
                        var rg = self._normalizeGenre(rGenres[rgi]);
                        if (!rg) continue;
                        var rgKey = self._genreMergeKey(rg);
                        if (!genreKeyMap[rgKey]) genreKeyMap[rgKey] = rg;
                        genreSet[genreKeyMap[rgKey]] = (genreSet[genreKeyMap[rgKey]] || 0) + 1;
                    }
                }
            }
        }
    ...
    if (!onProgress || streams.length <= BATCH_SIZE) {
