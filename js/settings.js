...
IPTVApp.prototype.initSettingsUI = function() {
    ...
    var debugRow = document.getElementById('debug-settings-row');
    if (debugRow) {
        if (this.isDevDevice()) {
    ...
    for (var i = 0; i < toggles.length; i++) {
        ...
        if (setting) {
            ...
            toggle.textContent = value ? I18n.t('settings.yes') : I18n.t('settings.no');
        }
    }
    ...
    for (var k = 0; k < options.length; k++) {
        ...
        if (this.settings[optSetting] === optValue) {
    ...
    var textSettings = ['tmdbApiKey', 'openSubtitlesApiKey', 'subDLApiKey'];
...
IPTVApp.prototype.initPairingCode = function() {
    ...
    if (statusEl) {
        statusEl.textContent = I18n.t('settings.waitingConfig');
        ...
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            if (data && data !== 'null') {
                try {
                    ...
                    window.log('Pairing parse error: ' + e.message);
                }
            }
        }
    };
...
IPTVApp.prototype.onPairingSuccess = function() {
    ...
    var statusEl = document.getElementById('settings-pairing-status');
    if (statusEl) {
        statusEl.textContent = I18n.t('settings.configReceived');
...
IPTVApp.prototype.handleSettingsSelect = function() {
    ...
    if (current.classList.contains('settings-toggle')) {
        ...
        if (!setting && current.querySelector('.toggle-option')) {
            ...
            current.textContent = this.settings[setting] ? I18n.t('settings.yes') : I18n.t('settings.no');
            if (setting === 'debugMode') {
                window.debugMode = this.settings[setting];
                var debugEl = document.getElementById('debug');
                if (debugEl && !this.settings[setting]) {
...
IPTVApp.prototype.renderPlaylistsList = function() {
    ...
    if (playlists.length === 0) {
        ...
        empty.textContent = I18n.t('settings.noPlaylists');
    ...
    for (var i = 0; i < playlists.length; i++) {
        ...
        statusEl.className = 'playlist-status checking';
        statusEl.textContent = '⏳';
        item.appendChild(statusEl);
        if (isActive) {
            var badge = document.createElement('span');
            badge.className = 'playlist-active-badge';
            badge.textContent = I18n.t('settings.active');
...
IPTVApp.prototype.showPlaylistEdit = function(playlistId) {
    ...
    if (playlistId) {
        for (var i = 0; i < this.settings.playlists.length; i++) {
            if (this.settings.playlists[i].id === playlistId) {
    ...
    titleEl.textContent = playlist ? I18n.t('settings.editPlaylist') : I18n.t('settings.newPlaylist');
...
IPTVApp.prototype.savePlaylist = function() {
    ...
    var found = false;
    for (var i = 0; i < this.settings.playlists.length; i++) {
        if (this.settings.playlists[i].id === playlist.id) {
...
IPTVApp.prototype.deletePlaylist = function() {
    ...
    var newPlaylists = [];
    for (var i = 0; i < this.settings.playlists.length; i++) {
        if (this.settings.playlists[i].id !== this.editingPlaylistId) {
            newPlaylists.push(this.settings.playlists[i]);
        }
    }
    this.settings.playlists = newPlaylists;
    if (this.settings.activePlaylistId === this.editingPlaylistId) {
...
IPTVApp.prototype.handlePlaylistsSelect = function() {
    ...
    var current = focusables[this.focusIndex];
    if (!current) return;
    var playlistId = current.dataset.playlistId ? parseInt(current.dataset.playlistId) : null;
    var action = current.dataset.action;
    if (action === 'select') {
        ...
        this.editingPlaylistId = playlistId;
        this.showConfirmModal(I18n.t('settings.confirmDeletePlaylist'), 'deletePlaylist');
    }
            ...
            if (xhr.readyState === 4) {
                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        window.log('loadM3UPlaylist: parse error: ' + e.message);
                        reject(e);
                    }
                }
                else {
...
IPTVApp.prototype.updatePlaylistValidationUI = function(playlistId, result) {
    var item = document.querySelector('.playlist-item[data-playlist-id="' + playlistId + '"]');
    if (!item) return;
...
IPTVApp.prototype.renderPatternCategories = function() {
    ...
    for (var i = 0; i < categories.length; i++) {
        ...
        editText.textContent = I18n.t('settings.editKeywords');
...
IPTVApp.prototype.openPatternEditor = function(section) {
    ...
    if (section === 'entertainment') {
        ...
        var subcatLabels = {
            concerts: '🎵 ' + I18n.t('home.concerts'),
            theatre: '🎭 ' + I18n.t('home.theatre'),
            spectacles: '🎪 ' + I18n.t('home.spectacles'),
            blindtest: '🎯 ' + I18n.t('home.blindtest'),
            karaoke: '🎤 ' + I18n.t('home.karaoke')
        };
        ...
        if (this.patternEditorCategory) {
            ...
            if (this.patternEditorIconType === 'emoji') {
                ...
                this.patternEditorCategory.icon = {
                    type: 'custom',
                    text: newText,
                    color: this.patternEditorIconColor || '#ffffff',
                    bg: this.patternEditorIconBg || '#000000'
                };
            }
...
IPTVApp.prototype.updatePatternIconPalettes = function() {
    ...
    if (textPalette) {
        ...
        for (var i = 0; i < textOptions.length; i++) {
            textOptions[i].classList.toggle('selected', textOptions[i].dataset.color === this.patternEditorIconColor);
        }
    }
    if (bgPalette) {
        var bgOptions = bgPalette.querySelectorAll('.color-option');
        for (var j = 0; j < bgOptions.length; j++) {
            bgOptions[j].classList.toggle('selected', bgOptions[j].dataset.color === this.patternEditorIconBg);
        }
    }
};

IPTVApp.prototype.updatePatternIconPreview = function() {
    var preview = document.getElementById('pattern-icon-preview');
    var iconInput = document.getElementById('pattern-category-icon');
    if (preview) {
        preview.textContent = iconInput.value || '';
        preview.style.color = this.patternEditorIconColor || '#ffffff';
        preview.style.background = this.patternEditorIconBg || '#000000';
    }
};
...
IPTVApp.prototype.renderPatternChips = function() {
    ...
    if (this.patternEditorSection === 'entertainment') {
        ...
        if (keywords.length === 0) {
            container.innerHTML = '<div style="color:#666;font-size:18px;padding:10px;">' + I18n.t('settings.noKeywords') + '</div>';
        }
    }
    else {
...
IPTVApp.prototype.updatePatternPreview = function() {
    ...
    previewList.innerHTML = '<div class="pattern-preview-item" style="color:#666;">' + I18n.t('loading') + '</div>';
    if (!this.api) {
        previewList.innerHTML = '<div class="pattern-preview-item" style="color:#666;">' + I18n.t('settings.noApiAvailable') + '</div>';
        return;
    }
    this.api.getVodCategories().then(function(categories) {
        self.renderPatternPreview(categories || []);
    }).catch(function() {
        previewList.innerHTML = '<div class="pattern-preview-item" style="color:#666;">' + I18n.t('settings.failedToLoadCategories') + '</div>';
    });
};
...
IPTVApp.prototype.renderPatternPreview = function(categories) {
    ...
    if (matchedCategories.length === 0) {
        previewList.innerHTML = '<div class="pattern-preview-item" style="color:#666;">' + I18n.t('settings.noMatchingCategories') + '</div>';
    }
    else {
        ...
        if (name) {
            ...
            if (this.iconType === 'custom') {
                ...
                iconData = {
                    ...
                    color: textColorEl ? textColorEl.dataset.color : '#ffffff',
                    bg: bgColorEl ? bgColorEl.dataset.color : '#1a1a1a'
                };
            }
            else {
...
IPTVApp.prototype.updateCustomIconPreview = function() {
    ...
    if (!preview) return;
    var text = document.getElementById('custom-icon-text').value || '';
    var textColorEl = document.querySelector('#text-color-palette .color-option.selected');
    var bgColorEl = document.querySelector('#bg-color-palette .color-option.selected');
...
IPTVApp.prototype.confirmDeleteCategory = function(categoryId) {
    ...
    this.showConfirmModal(I18n.t('settings.confirmDeleteCategory', { name: name }), function() {
        if (cat && cat.isDefault) {
