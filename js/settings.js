...
IPTVApp.prototype.showTTSVoiceModal = function() {
    ...
    loadingMsg.style.cssText = 'color: #888; padding: 20px';
    loadingMsg.textContent = I18n.t('common.loading', 'Loading...');
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            try {
                ...
                while (list.firstChild) list.removeChild(list.firstChild);
                var parseErrMsg = document.createElement('div');
                parseErrMsg.style.cssText = 'color: #ff5252; padding: 20px';
                parseErrMsg.textContent = I18n.t('common.error', 'Error');
                list.appendChild(parseErrMsg);
            }
        }
        else {
            while (list.firstChild) list.removeChild(list.firstChild);
            var httpErrMsg = document.createElement('div');
            httpErrMsg.style.cssText = 'color: #ff5252; padding: 20px';
            httpErrMsg.textContent = I18n.t('common.error', 'Error') + ': ' + xhr.status;
            list.appendChild(httpErrMsg);
        }
    };
    xhr.onerror = function() {
        while (list.firstChild) list.removeChild(list.firstChild);
        var connErrMsg = document.createElement('div');
        connErrMsg.style.cssText = 'color: #ff5252; padding: 20px';
        connErrMsg.textContent = I18n.t('common.connectionError', 'Connection error');
        list.appendChild(connErrMsg);
    };
    xhr.send();
};
...
IPTVApp.prototype.renderTTSVoiceList = function(container, voices, countries) {
    ...
    else if (this.ttsVoiceItems.length > 0) {
        this.ttsVoiceItems[0].classList.add('selected');
        this.settings.ttsVoice = this.ttsVoiceItems[0].dataset.voiceId;
        this.saveSettings();
        this.updateTTSVoiceButton();
    }
...
IPTVApp.prototype.updatePairingQR = function() {
    ...
    if (urlEl) {
        urlEl.textContent = 'https://iptv.blanquer.org';
    }
};
    ...
    for (var i = 0; i < inputs.length; i++) {
        ...
        if (!input.dataset.bound) {
            ...
            input.addEventListener('change', function() {
                ...
                if (id === 'proxyUrl') {
                    if (self.player) self.player.setProxyUrl(self.getStreamProxyUrl());
...
IPTVApp.prototype.handleSettingsSelect = function(clickedElement) {
    ...
    if (current.classList.contains('settings-toggle')) {
        ...
        document.getElementById('setting-' + btnSetting).textContent = this.formatSettingValue(btnSetting, this.settings[btnSetting]);
        this.saveSettings();
    }
        ...
        else if (actionType === 'validateLicense') {
            this.handleValidateLicense();
        }
...
IPTVApp.prototype.savePlaylist = function() {
    ...
    this.saveSettings();
    if (!found && this.settings.playlists.length === 1) {
...
IPTVApp.prototype._setPreviewMessage = function(container, text) {
    ...
    div.className = 'pattern-preview-item';
    div.style.color = '#666';
...
IPTVApp.prototype.updatePatternPreview = function() {
    ...
    this._setPreviewMessage(previewList, I18n.t('app.loading', 'Loading...'));
    if (!this.api) {
        this._setPreviewMessage(previewList, I18n.t('settings.noApiAvailable', 'No API available'));
        return;
    }
    Promise.all([
        ...
        self._setPreviewMessage(previewList, I18n.t('settings.failedToLoadCategories', 'Failed to load categories'));
    });
};
...
IPTVApp.prototype.renderPatternPreview = function(categories) {
    ...
    if (matchedCategories.length === 0) {
        self._setPreviewMessage(previewList, I18n.t('settings.noMatchingCategories', 'No matching categories'));
    }
    else {
...
IPTVApp.prototype.updateFreeboxVisibility = function() {
    ...
    var viaRow = document.getElementById('freebox-via-row');
    if (viaRow) {
...
IPTVApp.prototype.handleValidateLicense = function() {
    var self = this;
    var input = document.getElementById('setting-licenseCode');
