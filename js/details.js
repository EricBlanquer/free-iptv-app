...
IPTVApp.prototype.triggerFreeboxDownload = function() {
    ...
    if (this.getActiveStreamCount(playlistId) >= this.getMaxConnections(playlistId)) {
        if (!this._freeboxDownloadQueue) this._freeboxDownloadQueue = [];
        this._freeboxDownloadQueue.push({ url: url, filename: filename, streamId: streamId, playlistId: playlistId });
    ...
    this.startFreeboxDownload(url, filename, streamId, playlistId);
};
...
IPTVApp.prototype.triggerFreeboxSeasonDownload = function() {
    ...
    sorted.forEach(function(ep) {
        ...
        self._freeboxDownloadQueue.push({ url: url, filename: filename, streamId: epId, playlistId: playlistId });
        count++;
    });
    if (count === 0) {
    ...
    this.saveFreeboxMaps();
    window.log('Freebox season download: queued ' + count + ' episodes for S' + s);
...
IPTVApp.prototype.startFreeboxDownload = function(url, filename, streamId, playlistId) {
    var self = this;
...
IPTVApp.prototype.processDownloadQueue = function(downloads) {
    ...
    var keys = Object.keys(downloads);
    for (var i = 0; i < keys.length; i++) {
        if (downloads[keys[i]].status === 'downloading') activeCount++;
    }
    var queue = this._freeboxDownloadQueue || [];
    if (queue.length > 0) {
        var next = queue[0];
        ...
        if (providerActive < providerMax) {
            ...
            this.startFreeboxDownload(next.url, next.filename, next.streamId, next.playlistId);
    ...
    for (var j = 0; j < keys.length; j++) {
        ...
        if (st === 'downloading' || st === 'stopped' || st === 'error') { hasPausedOrActive = true; break; }
    }
...
IPTVApp.prototype.updateGlobalDownloadBar = function(downloads) {
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        else if ((dl.status === 'stopped' || dl.status === 'error') && this._freeboxDownloadMap && this._freeboxDownloadMap[dl.id]) {
            pausedCount++;
    ...
    if (activeDl) {
        ...
        if (activeCount > 1) {
            ...
            var totalPct = 0;
            for (var j = 0; j < keys.length; j++) {
                if (downloads[keys[j]].status === 'downloading') {
