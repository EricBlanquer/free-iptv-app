...
IPTVApp.prototype.showDetails = function(item) {
    ...
    // Only use streamId as seriesId if NOT from history/downloads (streamId could be episodeId)
    if (!seriesId && actualType === 'series' && !isFromHistory && !isFromDownload) {
        seriesId = streamId;
    }
    // Fallback for downloads without seriesId: search in watch history and progress data
    if (!seriesId && isFromDownload && actualType === 'series') {
        var epIdStr = String(streamId);
        for (var hi = 0; hi < this.watchHistory.length; hi++) {
            var hItem = this.watchHistory[hi];
            if ((hItem.episodeId == epIdStr || hItem.id == epIdStr) && hItem.seriesId) {
