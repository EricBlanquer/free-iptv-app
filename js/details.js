...
function setDescription(text, caller) {
    var el = document.getElementById('details-description');
    var old = el.textContent;
    window.log('DESC', caller + ': "' + (old || '').substring(0, 50) + '" → "' + (text || '').substring(0, 50) + '"');
    el.textContent = text;
}
...
IPTVApp.prototype.showDetails = function(item) {
    ...
    var isFromDownload = streamData && streamData._isDownload;
    var seriesId = this._resolveSeriesId(streamId, streamData, actualType, isFromHistory, isFromDownload);
    this.selectedStream = {
        id: streamId,
        type: actualType,
        data: streamData,
    ...
    if (actualType === 'vod' || streamType === 'vod') {
        this._showDetailsVod(streamId, streamData);
    } else if (actualType === 'series' || streamType === 'series') {
        this._showDetailsSeries(streamId, streamData, seriesId, isFromHistory, isFromDownload);
    }
    this.goToScreen('details', 'details', this.getDetailsPlayIndex());
};

IPTVApp.prototype._resolveSeriesId = function(streamId, streamData, actualType, isFromHistory, isFromDownload) {
    var seriesId = streamData ? (streamData.seriesId || streamData.series_id || streamData._seriesId) : null;
    if (!seriesId && actualType === 'series' && !isFromHistory && !isFromDownload) {
        seriesId = streamId;
    }
    if (!seriesId && isFromDownload && actualType === 'series') {
        seriesId = this._findSeriesIdFromProgress(String(streamId));
    }
    return seriesId;
};

IPTVApp.prototype._findSeriesIdFromProgress = function(epIdStr) {
    var epToSeries = {};
    for (var hi = 0; hi < this.watchHistory.length; hi++) {
        var hItem = this.watchHistory[hi];
        if (hItem.seriesId) {
            if (hItem.episodeId) epToSeries[String(hItem.episodeId)] = hItem.seriesId;
            if (hItem.id) epToSeries[String(hItem.id)] = hItem.seriesId;
        }
    }
    var seriesId = epToSeries[epIdStr] || null;
    if (!seriesId) {
        for (var epKey in this.episodeProgress) {
            if (this.episodeProgress[epKey].episodeId == epIdStr && this.episodeProgress[epKey].seriesId) {
                seriesId = this.episodeProgress[epKey].seriesId;
                break;
            }
        }
    }
    if (!seriesId) {
        for (var spKey in this.seriesProgress) {
            if (this.seriesProgress[spKey].episodeId == epIdStr && this.seriesProgress[spKey].seriesId) {
                seriesId = this.seriesProgress[spKey].seriesId;
                break;
            }
        }
    }
    if (seriesId) {
        window.log('showDetails: found seriesId=' + seriesId + ' from history/progress for episodeId=' + epIdStr);
    }
    return seriesId;
};

IPTVApp.prototype._resetDetailsUI = function(imageUrl, title, streamData) {
...
IPTVApp.prototype._findVersionByTag = function(versions, tag) {
    for (var pi = 0; pi < versions.length; pi++) {
...
IPTVApp.prototype._findBestQualityVersion = function(versions) {
    var bestScore = -1;
    var preferredIdx = 0;
    for (var qi = 0; qi < versions.length; qi++) {
        var tag = (versions[qi].tag || '').toUpperCase();
        var score = 0;
    ...
    window.log('showDetails: auto-selected best quality idx=' + preferredIdx + ' tag=' + (versions[preferredIdx].tag || 'default'));
    return preferredIdx;
};
...
IPTVApp.prototype._showDetailsSeries = function(streamId, streamData, seriesId, isFromHistory, isFromDownload) {
    var playBtn = document.getElementById('play-btn');
    var continueBtn = document.getElementById('continue-btn');
    var markWatchedBtn = document.getElementById('mark-watched-btn');
...
IPTVApp.prototype.displayDualRatings = function(metaEl, hasParts, providerRating, tmdbRating, voteCount) {
    ...
    if (voteCount > 0) {
        ...
        countSpan.textContent = ' (' + this.formatVoteCount(voteCount) + ')';
        metaEl.appendChild(countSpan);
    }
};

IPTVApp.prototype.formatVoteCount = function(count) {
...
IPTVApp.prototype.updateSeriesContinueButton = function(seriesData) {
    ...
    var pId = currentPlaylistId || (this.currentPlayingStream && this.currentPlayingStream._playlistId) || this.settings.activePlaylistId || '';
    if (seriesData.episodes) {
        ...
        for (var si = 0; si < seasonKeys.length; si++) {
            ...
            for (var ei = 0; ei < episodes.length; ei++) {
                var ep = episodes[ei];
                var epKey = pId + '_' + ep.id;
                var progress = this.episodeProgress[epKey] || null;
    ...
    if (lastWatched && seriesData.episodes) {
        var analysis = this._analyzeSeriesProgress(seriesData, lastWatched.season, lastWatched.episode);
        var newEpisodesCount = analysis.newCount;
        ...
        if (newEpisodesCount > 0) {
            var nextEpisode = analysis.nextEpisode;
            if (nextEpisode) {
...
IPTVApp.prototype._analyzeSeriesProgress = function(seriesData, lastSeason, lastEpisode) {
...
IPTVApp.prototype.countNewEpisodes = function(seriesData, lastSeason, lastEpisode) {
    return this._analyzeSeriesProgress(seriesData, lastSeason, lastEpisode).newCount;
};

IPTVApp.prototype.findNextEpisodeAfter = function(seriesData, lastSeason, lastEpisode) {
    return this._analyzeSeriesProgress(seriesData, lastSeason, lastEpisode).nextEpisode;
};
...
IPTVApp.prototype.selectSeason = function(seasonNum) {
    ...
    if (this.tmdbInfo && this.tmdbInfo.seasons) {
        ...
        if (seasonInfo && seasonInfo.overview) {
            setDescription(seasonInfo.overview, 'selectSeason');
            this.preloadTTS(seasonInfo.overview);
            this.showTTSTooltip();
        }
        else if (this.tmdbInfo.overview) {
            setDescription(this.tmdbInfo.overview, 'selectSeason-fallback');
...
IPTVApp.prototype.titleSimilarity = function(a, b) {
    ...
    return Math.round((2 * intersection / (countA + countB)) * 100);
};
...
IPTVApp.prototype.trimTMDBResult = function(data) {
    ...
    if (data.credits) {
        ...
        if (data.credits.cast) {
            trimmed.credits.cast = data.credits.cast.map(function(c) {
...
IPTVApp.prototype.fetchTMDBInfo = function(title, type) {
    ...
    if (!TMDB.isEnabled()) {
        setDescription(I18n.t('details.configureTMDB', 'Configure TMDB API key in settings to view descriptions.'), 'fetchTMDB-noKey');
        return;
    }
    setDescription(I18n.t('messages.searching', 'Searching...', { title: cleanTitle }), 'fetchTMDB-searching');
            ...
            if (result) {
                ...
                TMDB.getTVDetails(tmdbId, function(tvResult) {
                    if (tvResult) {
                        ...
                        setDescription(I18n.t('details.noDescription', 'No description'), 'fetchTMDBById-movie-notFound');
                    }
                });
            }
        });
            ...
            if (result) {
                ...
                TMDB.getMovieDetails(tmdbId, function(movieResult) {
                    if (movieResult) {
                        ...
                        setDescription(I18n.t('details.noDescription', 'No description'), 'fetchTMDBById-tv-notFound');
                    }
                });
            }
        });
...
IPTVApp.prototype.displayTMDBDetails = function(result, type) {
    ...
    setDescription(descText, 'displayTMDBDetails');
    if (result.overview) {
...
IPTVApp.prototype.downloadSeasonEpisodes = function(episodes) {
    ...
    sorted.forEach(function(ep) {
        ...
        if (batchMode) {
            self.startFreeboxDownload(url, filename, epId, playlistId, poster, seriesId);
        } else {
            self._freeboxDownloadQueue.push({ url: url, filename: filename, streamId: epId, playlistId: playlistId, poster: poster, seriesId: seriesId });
        }
        count++;
    });
    if (count === 0) {
...
IPTVApp.prototype.cancelActiveDownload = function(streamId, playlistId) {
    ...
    if (String(downloadId).indexOf('vm_') === 0) {
        var vmId = String(downloadId).replace('vm_', '');
        var baseProxy = self.settings.proxyUrl.replace(/\/+$/, '');
        var xhr = new XMLHttpRequest();
        xhr.open('GET', baseProxy + '/download/cancel?id=' + encodeURIComponent(vmId) + proxyDuidParam(), true);
        xhr.timeout = 10000;
        xhr.onload = function() {
        ...
        xhr.onerror = function() {
            window.log('VM cancel error');
            self.showToast(I18n.t('freebox.downloadError', 'Download error'), 3000, true);
        };
        xhr.send();
        return;
    }
    ...
    if (this.settings.freeboxDownloadViaProxy && this.settings.proxyEnabled && this.settings.proxyUrl) {
        ...
        xhr.onload = function() {
            try {
                ...
                var vmList = (resp.success && resp.result) ? resp.result : [];
                var fakeDownloads = vmList.map(function(dl) {
                    return { id: 'vm_' + dl.id, name: dl.name, status: dl.status, rx_bytes: dl.rx_bytes, size: dl.size, rx_rate: dl.rx_rate };
                });
                window.log('Downloads screen: VM returned ' + fakeDownloads.length + ' downloads');
                var hasActive = fakeDownloads.some(function(d) { return d.status === 'downloading' || d.status === 'uploading' || d.status === 'queued'; });
                if (hasActive) self.ensureFreeboxPolling();
...
IPTVApp.prototype.renderDownloadsList = function(freeboxDownloads, queueSnapshot) {
    ...
    var seriesMap = this._freeboxDownloadSeriesMap || {};
    var freeboxStreamIds = {};
    for (var i = 0; i < freeboxDownloads.length; i++) {
        var dl = freeboxDownloads[i];
        if (!dl.name && dl.size === 0) continue;
        if (dl.status === 'done' || dl.status === 'stopped' || dl.status === 'error') continue;
        var isVmDownload = String(dl.id).indexOf('vm_') === 0;
        if (!isVmDownload && !dlMap[dl.id]) continue;
        ...
        var statusLabel = dl.status === 'done' ? '✓' :
            ...
            dl.status === 'uploading' ? '↗ ' + (pct > 0 ? pct + '%' : 'Freebox...') :
        ...
        items.push({
            ...
            series_id: seriesMap[dl.id] || null,
            name: dlName,
    ...
    items.sort(function(a, b) {
        var orderA = DL_STATUS_ORDER[a._dlStatus] !== undefined ? DL_STATUS_ORDER[a._dlStatus] : DL_STATUS_LAST;
        var orderB = DL_STATUS_ORDER[b._dlStatus] !== undefined ? DL_STATUS_ORDER[b._dlStatus] : DL_STATUS_LAST;
        if (orderA !== orderB) return orderA - orderB;
        var idA = a._dlId ? parseInt(String(a._dlId).replace('vm_', '')) : DL_STATUS_LAST;
        var idB = b._dlId ? parseInt(String(b._dlId).replace('vm_', '')) : DL_STATUS_LAST;
...
IPTVApp.prototype.removeDownloadAtIndex = function(index) {
    ...
    if (stream._dlStatus === 'queued' && !stream._dlId) {
        ...
        var vmDlId = stream._dlId;
        var vmId = String(vmDlId).replace('vm_', '');
        var baseProxy = self.settings.proxyUrl.replace(/\/+$/, '');
        var xhr = new XMLHttpRequest();
        xhr.open('GET', baseProxy + '/download/cancel?id=' + encodeURIComponent(vmId) + proxyDuidParam(), true);
        xhr.timeout = 10000;
        xhr.onload = function() {
            if (self._freeboxDownloadMap) delete self._freeboxDownloadMap[vmDlId];
            if (self._freeboxDownloadProviderMap) delete self._freeboxDownloadProviderMap[vmDlId];
            self.saveFreeboxMaps();
            self.showToast(I18n.t('freebox.downloadCancelled', 'Download cancelled'), 2000);
            self.updateGlobalDownloadBar();
        };
        xhr.onerror = function() {
            window.log('VM delete error');
        };
        xhr.send();
    }
...
IPTVApp.prototype.removeCompletedDownloadsFromGrid = function(downloads) {
    ...
    for (var i = 0; i < gridItems.length; i++) {
        ...
        var isVmDl = String(stream._dlId).indexOf('vm_') === 0;
        if (isVmDl) {
            if (!downloads[stream._dlId]) toRemove.push(i);
        }
        else if (!dlMap[stream._dlId]) {
            toRemove.push(i);
        }
    }
...
IPTVApp.prototype.startFreeboxDownload = function(url, filename, streamId, playlistId, poster, seriesId) {
    ...
    FreeboxAPI.addDownload(url, filename).then(function(result) {
        ...
        if (seriesId) {
            if (!self._freeboxDownloadSeriesMap) self._freeboxDownloadSeriesMap = {};
            self._freeboxDownloadSeriesMap[result.id] = seriesId;
        }
        self.saveFreeboxMaps();
...
IPTVApp.prototype._startVmDownload = function(url, filename, streamId, playlistId, poster, seriesId) {
    if (!this._vmSendQueue) this._vmSendQueue = [];
    this._vmSendQueue.push({ url: url, filename: filename, streamId: streamId, playlistId: playlistId, poster: poster, seriesId: seriesId });
    if (!this._vmSending) {
        this._processVmSendQueue();
    }
};

IPTVApp.prototype._processVmSendQueue = function() {
    if (!this._vmSendQueue || this._vmSendQueue.length === 0) {
        this._vmSending = false;
        return;
    }
    this._vmSending = true;
    var self = this;
    var item = this._vmSendQueue.shift();
    var baseProxy = this.settings.proxyUrl.replace(/\/+$/, '');
    var maxConn = this.getMaxConnections(item.playlistId);
    var dlUrl = baseProxy + '/download?url=' + encodeURIComponent(item.url) + '&filename=' + encodeURIComponent(item.filename) + '&max=' + maxConn;
    if (this.settings.freeboxAppToken) {
        dlUrl += '&freebox_host=' + encodeURIComponent(this.settings.freeboxHost || 'mafreebox.freebox.fr');
        dlUrl += '&freebox_app_token=' + encodeURIComponent(this.settings.freeboxAppToken);
        var proxyUrlParsed = this.settings.proxyUrl.replace(/^https?:\/\//, '').replace(/\/+$/, '');
        dlUrl += '&vm_host=' + encodeURIComponent(proxyUrlParsed);
    }
    dlUrl += proxyDuidParam();
    window.log('VM download: ' + item.url + ' -> ' + item.filename + ' (streamId=' + item.streamId + ' playlistId=' + item.playlistId + ')');
    var xhr = new XMLHttpRequest();
    ...
    xhr.onload = function() {
        try {
            var resp = JSON.parse(xhr.responseText);
            if (!resp.success) {
                window.log('VM download error: ' + (resp.msg || 'unknown'));
                self.showToast(I18n.t('freebox.downloadError', 'Download error') + ': ' + (resp.msg || 'unknown'), 5000, true);
            }
            else {
                var vmId = resp.result.id;
                window.log('VM download registered: vmId=' + vmId + ' streamId=' + item.streamId);
                if (!self._freeboxDownloadMap) self._freeboxDownloadMap = {};
                self._freeboxDownloadMap['vm_' + vmId] = item.streamId;
                if (!self._freeboxDownloadProviderMap) self._freeboxDownloadProviderMap = {};
                self._freeboxDownloadProviderMap['vm_' + vmId] = item.playlistId;
                if (item.poster) {
                    if (!self._freeboxDownloadPosterMap) self._freeboxDownloadPosterMap = {};
                    self._freeboxDownloadPosterMap['vm_' + vmId] = item.poster;
                }
                if (item.seriesId) {
                    if (!self._freeboxDownloadSeriesMap) self._freeboxDownloadSeriesMap = {};
                    self._freeboxDownloadSeriesMap['vm_' + vmId] = item.seriesId;
                }
                self.saveFreeboxMaps();
                self.updateDownloadButton();
                self.ensureFreeboxPolling();
            ...
            window.log('VM download parse error: ' + ex.message);
        }
        self._processVmSendQueue();
    };
    xhr.onerror = function() {
        window.log('VM download network error');
        self._processVmSendQueue();
    };
    xhr.ontimeout = function() {
        window.log('VM download timeout');
        self._processVmSendQueue();
    };
    xhr.send();
};
...
IPTVApp.prototype.cleanupDownloadMap = function(downloads) {
    ...
    for (var j = 0; j < dlKeys.length; j++) {
        ...
        if (status !== 'downloading' && status !== 'uploading' && status !== 'queued' && status !== 'stopped' && status !== 'error') {
...
IPTVApp.prototype._startVmPolling = function() {
    var self = this;
    if (this._vmPollTimer) return;
    var baseProxy = this.settings.proxyUrl.replace(/\/+$/, '');
    this._vmPollTimer = setInterval(function() {
        var xhr = new XMLHttpRequest();
        xhr.open('GET', baseProxy + '/downloads' + '?' + proxyDuidParam().replace(/^&/, ''), true);
        xhr.timeout = 10000;
        xhr.onload = function() {
            try {
                ...
                window.log('VM poll parse error: ' + ex.message);
            }
        };
        xhr.onerror = function() {
            window.log('VM poll network error');
        };
        xhr.send();
    }, 2000);
};

IPTVApp.prototype._stopVmPolling = function() {
    if (this._vmPollTimer) {
        clearInterval(this._vmPollTimer);
        this._vmPollTimer = null;
    }
};
...
IPTVApp.prototype.processDownloadQueue = function(downloads) {
    ...
    for (var j = 0; j < keys.length; j++) {
        ...
        var isTracked = dlMap[downloads[keys[j]].id] || (isVmMode && String(downloads[keys[j]].id).indexOf('vm_') === 0);
        if ((st === 'downloading' || st === 'uploading' || st === 'stopped' || st === 'error') && isTracked) { hasPausedOrActive = true; break; }
    }
...
IPTVApp.prototype.updateGlobalDownloadBar = function(downloads) {
    ...
    var totalActive = activeCount + uploadCount;
    ...
    if (activeDl) {
        ...
        if (activeCount === 0 && uploadCount === 1) {
            displayName = '↗ ' + displayName;
        }
        nameEl.textContent = displayName;
        var dlPct = activeDl.rx_pct || (activeDl.size > 0 ? Math.round((activeDl.rx_bytes / activeDl.size) * 100) : 0);
        if (dlPct > 0) {
            pctEl.textContent = dlPct + '%';
        }
        else if (activeDl.rx_bytes > 0) {
            pctEl.textContent = Math.round(activeDl.rx_bytes / 1048576) + ' Mo';
        }
        else {
            pctEl.textContent = '...';
        }
        fillEl.style.width = dlPct + '%';
        fillEl.style.background = '#4CAF50';
        if (totalActive > 1) {
            var parts = [];
            if (activeCount > 0) {
                parts.push(activeCount + ' ' + I18n.t('freebox.downloadsActive', 'downloads'));
            }
            if (uploadCount > 0) {
                parts.push(uploadCount + ' upload' + (uploadCount > 1 ? 's' : ''));
            }
            ...
            var pctCount = 0;
            for (var j = 0; j < keys.length; j++) {
                var djDl = downloads[keys[j]];
                if (djDl.status === 'downloading' && (dlMap[djDl.id] || (isVmMode && String(djDl.id).indexOf('vm_') === 0))) {
                    totalPct += (djDl.rx_pct || (djDl.size > 0 ? Math.round((djDl.rx_bytes / djDl.size) * 100) : 0));
                    pctCount++;
                }
            }
            if (pctCount > 0) {
                pctEl.textContent = Math.round(totalPct / pctCount) + '%';
                fillEl.style.width = Math.round(totalPct / pctCount) + '%';
            }
        }
    }
    else if (pausedDl) {
