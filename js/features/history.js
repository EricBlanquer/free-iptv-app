...
IPTVApp.prototype.showContinueInGrid = function() {
    ...
    if (!viewModes || Array.isArray(viewModes)) viewModes = {};
    var viewMode = viewModes[this.currentSection] || 'grid';
    ...
    for (var sKey in this.seriesProgress) {
        ...
        if (sProg.episodeId && sProg.playlistId && sProg.seriesId) {
            epToSeries[sProg.playlistId + '_' + sProg.episodeId] = sProg.seriesId;
        }
    }
...
IPTVApp.prototype.showContinueScreen = function() {
    ...
    for (var i = 0; i < this.watchHistory.length; i++) {
        ...
        if (activePlaylistId && item.playlistId && String(item.playlistId) !== String(activePlaylistId)) continue;
    ...
    var historyById = {};
    var historyBySeriesId = {};
    for (var hi = 0; hi < this.watchHistory.length; hi++) {
        var h = this.watchHistory[hi];
        var hKey = h.playlistId + '_' + h.id;
        if (!historyById[hKey]) historyById[hKey] = h;
        if (h.episodeId) {
            var heKey = h.playlistId + '_' + h.episodeId;
            if (!historyById[heKey]) historyById[heKey] = h;
        }
        if (h.seriesId) {
            var hsKey = h.playlistId + '_' + h.seriesId;
            if (!historyBySeriesId[hsKey]) historyBySeriesId[hsKey] = h;
        }
    }
    var seriesMap = {};
    for (var epKey in this.episodeProgress) {
        ...
        if (epProg.position >= minMs && !epProg.watched) {
            ...
            var epHistory = historyById[epPlaylistId + '_' + epId] || null;
    ...
    for (var sKey in this.seriesProgress) {
        if (!seriesMap[sKey]) {
            ...
            var sHistory = historyBySeriesId[sPlaylistId + '_' + sSeriesId] || null;
            if (sHistory) {
                seriesMap[sKey] = {
                    id: sSeriesId,
                    seriesId: sSeriesId,
