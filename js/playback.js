...
IPTVApp.prototype.updatePlayerTracks = function() {
    ...
    var subtitleToggle = document.getElementById('player-subtitle-toggle');
    if (tracks.subtitle.length > 0) {
        ...
        this.setHidden(subtitleToggle, false);
        visibleButtons++;
        this.updateSubtitleToggleIcon();
    }
    else {
        this.setHidden(subtitleBtn, true);
        this.setHidden(subtitleToggle, true);
    }
...
IPTVApp.prototype.toggleSubtitles = function() {
    if (this.player.subtitlesEnabled) {
        ...
        if (restoreIdx !== undefined && restoreIdx >= 0 && tracks && tracks.subtitle[restoreIdx]) {
            ...
            this.player.showSubtitles();
            document.getElementById('player-subtitle-label').textContent = 'Externe';
        }
...
IPTVApp.prototype.updateSubtitleToggleIcon = function() {
    var label = document.getElementById('subtitle-toggle-label');
    if (!label) return;
    if (label) {
        label.textContent = this.player.subtitlesEnabled ? 'SUB ✓' : 'SUB ✗';
    }
    var toggle = document.getElementById('player-subtitle-toggle');
    if (toggle) {
        toggle.classList.toggle('subtitle-off', !this.player.subtitlesEnabled);
    }
};
...
IPTVApp.prototype.selectPlayerTrack = function() {
    ...
    else if (btn.id === 'player-subtitle-toggle') {
