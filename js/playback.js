...
IPTVApp.prototype.updatePlayerTracks = function() {
    ...
    if (tracks.subtitle.length > 0) {
        ...
        var subLabel = subIdx === -1 ? I18n.t('player.disabled', 'Disabled') : (subIdx === -2 ? 'Externe' : (tracks.subtitle[subIdx] ? tracks.subtitle[subIdx].language : 'Sous-titres'));
...
IPTVApp.prototype.getSeriesSubtitlePref = function() {
    var stream = this.currentPlayingStream;
    if (!stream || !stream.series_id) return null;
    var key = 'subPref_' + (stream._playlistId || this.settings.activePlaylistId) + '_' + stream.series_id;
...
IPTVApp.prototype.saveSeriesSubtitlePref = function(lang) {
    var stream = this.currentPlayingStream;
    if (!stream || !stream.series_id) return;
    var key = 'subPref_' + (stream._playlistId || this.settings.activePlaylistId) + '_' + stream.series_id;
    localStorage.setItem(key, JSON.stringify({ lang: lang }));
    window.log('SUBTITLE', 'saved series pref: ' + lang + ' for series ' + stream.series_id);
};

IPTVApp.prototype.autoSelectSubtitleTrack = function(subtitleTracks) {
    // Check series-level subtitle preference first
    var seriesPref = this.getSeriesSubtitlePref();
    if (seriesPref) {
        if (seriesPref.lang === 'disabled') {
            window.log('autoSelectSubtitleTrack: series pref is disabled');
            this.currentSubtitleIndex = -1;
