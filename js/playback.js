...
IPTVApp.prototype.playStream = function(streamId, type, stream, startPosition) {
    ...
    if (stream && stream.url) {
        ...
        var apisCount = this.apis ? this.apis.length : 0;
        window.log('playStream: looking for API, stream._playlistId=' + (stream ? stream._playlistId : 'null') + ', apis.length=' + apisCount);
        if (stream && stream._playlistId && this.apis && this.apis.length > 1) {
            ...
            for (var i = 0; i < this.apis.length; i++) {
                ...
                if (String(apiPid) === String(streamPlaylistId)) {
...
IPTVApp.prototype.onPlaybackCompleted = function() {
    ...
    if (isCatchup) {
        window.log('onPlaybackCompleted: catchup, calling playNextCatchup');
    ...
    if (isSeries) {
        ...
        if (this.pendingSeriesInfoPromise && !this.currentSeriesInfo) {
            window.log('onPlaybackCompleted: waiting for series info...');
            this.pendingSeriesInfoPromise.then(function() {
                window.log('onPlaybackCompleted: series info loaded, proceeding');
                self.proceedWithNextEpisode();
            }).catch(function(err) {
                window.log('onPlaybackCompleted: series info failed: ' + err);
                self.proceedWithNextEpisode();
            });
        }
        else {
...
IPTVApp.prototype.proceedWithNextEpisode = function() {
    ...
    if (this.currentSeriesInfo && this.currentSeriesInfo.episodes) {
        window.log('proceedWithNextEpisode: seasons=' + Object.keys(this.currentSeriesInfo.episodes).join(','));
        var eps = this.currentSeriesInfo.episodes[this.currentSeason];
        if (eps) {
            window.log('proceedWithNextEpisode: season ' + this.currentSeason + ' has ' + eps.length + ' episodes, nums=' + eps.map(function(e) { return e.episode_num; }).join(','));
        }
    }
...
IPTVApp.prototype.updatePlayerProgress = function(current, total) {
    ...
    if (this.currentPlayingType === 'catchup' && this.catchupParams) {
        ...
        if (!this._catchupLogDone) {
            ...
            window.log('Catchup progress: current=' + current + ' programDuration=' + programDuration);
        }
        // Update seek position only when stream is ready
        if (this.streamReady) {
            this.seekTargetPosition = current;
        }
        var percent = programDuration > 0 ? (current / programDuration) * 100 : 0;
        percent = Math.min(100, percent);
        document.getElementById('progress-bar').style.width = percent + '%';
        var remaining = Math.max(0, programDuration - current);
        ...
        // Detect near-end for catchup: if within 5 seconds of program end, play next
        // Ignore during first 5 seconds after loading to avoid false triggers
        var timeSinceStart = Date.now() - (this._catchupStartTime || 0);
        if (current > 0 && (programDuration - current) < 5000 && !this._completionTriggered && timeSinceStart > 5000) {
            window.log('Catchup near end: current=' + current + ' duration=' + programDuration + ', playing next');
            this._completionTriggered = true;
            this.playNextCatchup();
        }
        return;
    ...
    if (isLive) {
        ...
        if (this.player.isInTimeshift && liveBufferEl) {
            ...
            if (bufferInfo.available && bufferInfo.seconds > 0) {
                liveBufferEl.textContent = I18n.t('player.pauseDuration', 'Pause') + ': ' + this.player.formatTime(bufferInfo.seconds * 1000) + ' (' + I18n.t('player.returnToLive', '→ Live') + ')';
...
IPTVApp.prototype.autoSelectSubtitleTrack = function(subtitleTracks) {
    ...
    window.log('autoSelectSubtitleTrack: defaultLang=' + defaultLang + ' codes=' + targetLangs.join(','));
    // Find first subtitle matching target language (excluding forced)
    ...
    window.log('autoSelectSubtitleTrack: no match found');
    return false;
};
...
IPTVApp.prototype.playPrevCatchup = function() {
    ...
    var end = parseInt(prog.stop_timestamp, 10);
    var duration = Math.round((end - start) / 60);
    window.log('Catchup: playing previous program index=' + prevIndex);
