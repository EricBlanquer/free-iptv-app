...
class TVPlayer {
    ...
    _playDirect(url) {
        try {
            ...
            webapis.avplay.prepareAsync(() => {
                window.log('PLAYER', 'prepareAsync done, seeking to ' + this.startPosition + 'ms');
                // Always seek - AVPlay can retain previous position and getCurrentTime() returns 0 here
    ...
    playNative(url) {
        ...
        try {
            ...
            webapis.avplay.setListener(this._getListeners());
            webapis.avplay.prepareAsync(() => {
                window.log('PLAYER', 'playNative prepareAsync done, seeking to ' + this.startPosition + 'ms');
                // Always seek - AVPlay can retain previous position and getCurrentTime() returns 0 here
                webapis.avplay.seekTo(this.startPosition);
                webapis.avplay.play();
                this.isPlaying = true;
    ...
    playHtml5(url) {
        ...
        this.videoElement.onloadedmetadata = function() {
            ...
            if (self.startPosition > 0) {
                window.log('PLAYER', 'HTML5 seeking to start position: ' + self.startPosition + 'ms');
    ...
    _getListeners() {
        return {
            onbufferingstart: () => {
                ...
                window.log('PLAYER', 'Buffering started @' + pos + 'ms');
