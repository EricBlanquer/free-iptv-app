...
class TVPlayer {
    ...
    play(url, isLive = false, startPosition = 0) {
        ...
        if (this.proxyUrl) {
            ...
            var proxiedUrl = baseUrl + '/?url=' + encodeURIComponent(url);
            ...
            if (isLive && this._needsTranscode(url)) {
                ...
                window.log('HTTP', 'Stream proxy (transcode): ' + proxiedUrl);
            }
            else {
                window.log('HTTP', 'Stream proxy: ' + proxiedUrl);
            }
    ...
    _playDirect(url, retryCount = 0) {
        ...
        try {
            ...
            webapis.avplay.prepareAsync(() => {
                ...
                webapis.avplay.play();
                // Seek AFTER play - AVPlay ignores seekTo before play() starts
    ...
    playHtml5(url) {
        ...
        this.videoElement.onloadedmetadata = function() {
            ...
            if (self.startPosition > 0) {
                window.log('PLAYER', 'HTML5 seeking to start position: ' + formatMs(self.startPosition));
