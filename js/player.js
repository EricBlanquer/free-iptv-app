...
class TVPlayer {
    ...
    _startFreezeDetection() {
        ...
        this.freezeCheckInterval = setInterval(function() {
            ...
            if (timeDiff < 500) {
                ...
                window.log('Freeze check: time not advancing (' + self.freezeCheckCount + '/3)');
    ...
    _resolveAndPlay(url) {
        ...
        this.resolveRedirects(url, function(finalUrl) {
            if (finalUrl !== url) {
                window.log('Playing resolved URL');
            }
    ...
    resolveRedirects(url, callback) {
        window.log('Resolving redirects for: ' + url.substring(url.lastIndexOf('/') + 1));
        var done = false;
        var xhr = new XMLHttpRequest();
        xhr.open('GET', url, true);
        xhr.timeout = 5000;
        xhr.onreadystatechange = function() {
            window.log('XHR state: ' + xhr.readyState + ' status: ' + xhr.status);
            // As soon as we get headers (readyState 2+), we can check responseURL
            if (!done && xhr.readyState >= 2 && xhr.responseURL) {
                done = true;
                var finalUrl = xhr.responseURL;
                xhr.abort();
                if (finalUrl !== url) {
                    window.log('Redirect: ' + finalUrl);
                }
                callback(finalUrl);
            }
        };
        xhr.onerror = function() {
            if (!done) {
                ...
                window.log('XHR error, using original URL');
                callback(url);
            }
        };
        xhr.ontimeout = function() {
            if (!done) {
                ...
                window.log('XHR timeout, using original URL');
                callback(url);
            }
        };
        try {
            ...
            if (!done) {
                ...
                window.log('XHR exception: ' + e.message);
                callback(url);
            }
        }
    }
    ...
    play(url, isLive = false, startPosition = 0) {
        window.log('PLAYER play ' + (isLive ? 'LIVE ' : '') + url + (startPosition > 0 ? ' @' + startPosition + 'ms' : ''));
        var self = this;
        this.currentUrl = url;
        ...
        this._stopFreezeDetection();
        // For VOD/series URLs, resolve redirects first (AVPlay doesn't follow HTTP 302)
        var needsRedirectResolve = url.indexOf('/series/') !== -1 || url.indexOf('/movie/') !== -1;
        if (needsRedirectResolve) {
            window.log('Resolving redirect for VOD/series...');
            this._resolveAndPlay(url);
            return;
        }
    ...
    _playDirect(url) {
        try {
            ...
            // Close any existing AVPlay session before opening new one
            try {
                webapis.avplay.stop();
            ...
            webapis.avplay.prepareAsync(() => {
                // Seek to start position BEFORE play to avoid double buffering
                if (this.startPosition > 0) {
                    window.log('PLAYER seeking to start position: ' + this.startPosition + 'ms');
                    webapis.avplay.seekTo(this.startPosition);
                }
                webapis.avplay.play();
                this.isPlaying = true;
                this.isPaused = false;
    ...
    playHtml5(url) {
        ...
        this.videoElement.onloadedmetadata = function() {
            ...
            // Seek to start position to avoid double buffering
            if (self.startPosition > 0) {
                window.log('HTML5 seeking to start position: ' + self.startPosition + 'ms');
    ...
    seekTo(milliseconds) {
        try {
            ...
            window.log('SeekTo: requested=' + milliseconds + ' duration=' + this.duration + ' safePos=' + safePos);
            if (this.useHtml5 && this.videoElement) {
            ...
            if (typeof webapis !== 'undefined' && webapis.avplay) {
                ...
                if (!width || !height) {
                    try {
                        ...
                        window.log('ERROR getTotalTrackInfo: ' + ex2);
                    }
                }
                if (width && height) {
    ...
    getTracks() {
        ...
        try {
            if (typeof webapis !== 'undefined' && webapis.avplay) {
                var tracks = webapis.avplay.getTotalTrackInfo();
                window.log('getTracks raw: ' + JSON.stringify(tracks));
    ...
    setSubtitleSync(offsetMs) {
        window.log('setSubtitleSync: ' + offsetMs + 'ms');
        try {
            if (typeof webapis !== 'undefined' && webapis.avplay) {
    ...
    setDisplayMode(mode) {
        window.log('setDisplayMode: ' + mode);
    ...
    // Set buffering timeout (ms) - 0 to disable
    setBufferingTimeout(timeoutMs) {
        window.log('setBufferingTimeout: ' + timeoutMs + 'ms');
        try {
            if (typeof webapis !== 'undefined' && webapis.avplay) {
                webapis.avplay.setTimeoutForBuffering(timeoutMs);
                return true;
            }
        }
        catch (e) {
            window.log('ERROR setBufferingTimeout: ' + e.message);
        }
        return false;
    }
