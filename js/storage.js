...
IPTVApp.prototype.updateEpisodeProgress = function(episodeId, position, duration, playlistId) {
...
IPTVApp.prototype.markVodAsCompleted = function(streamId) {
    ...
    return this.loadProviderCacheLocal(playlistId).then(function(localData) {
        ...
        if (typeof DevRemoteSync !== 'undefined' && DevRemoteSync.isDevDevice) {
            return DevRemoteSync.loadProviderCache(playlistId).then(function(remoteCache) {
                if (remoteCache && remoteCache.timestamp && remoteCache.data) {
                    var ageMinutes = Math.round((Date.now() - remoteCache.timestamp) / 60000);
                    var needsRefresh = Date.now() - remoteCache.timestamp > PROVIDER_CACHE_TTL;
                    if (needsRefresh) {
                        window.log('Provider cache REMOTE stale for ' + playlistId + ' (age: ' + ageMinutes + 'min), will refresh in background');
                        remoteCache.data._needsRefresh = true;
                        remoteCache.data._playlistId = playlistId;
                    }
                    else {
                        window.log('Provider cache hit from REMOTE for ' + playlistId + ' (age: ' + ageMinutes + 'min)');
                    }
                ...
                window.log('Provider cache REMOTE miss for ' + playlistId);
                return null;
            });
        }
        return null;
            ...
            if (typeof DevRemoteSync !== 'undefined' && DevRemoteSync.isDevDevice) {
                return DevRemoteSync.loadProviderCache(id).then(function(remoteCache) {
                    if (remoteCache && remoteCache.timestamp && remoteCache.data) {
                        ...
                        if (needsRefresh) {
                            remoteCache.data._needsRefresh = true;
                            remoteCache.data._playlistId = id;
                        }
                        window.log('loadMergedProviderCache: got remote cache for ' + id + (needsRefresh ? ' (stale)' : ''));
