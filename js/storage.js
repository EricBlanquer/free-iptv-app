...
IPTVApp.prototype.saveTMDBCache = function() {
    var self = this;
    this.initTMDBCacheDB().then(function(db) {
        if (!db) return;
        try {
            ...
            window.log('ERROR', 'saveTMDBCache: ' + e.message);
        }
    });
};
...
IPTVApp.prototype.updateCacheInfoDisplay = function() {
    var cacheInfoEl = document.getElementById('cache-info');
    if (!cacheInfoEl) return;
    if (!this.providerCacheInfo) {
        cacheInfoEl.textContent = '';
        return;
    }
    var info = this.providerCacheInfo;
    var sourceLabel = info.source === 'provider' ? 'Provider' : 'Cache';
    var dateStr = '-';
    if (info.timestamp) {
    ...
    var text = I18n.t('settings.dataSource', 'Data source') + colon + sourceLabel + ' (' + dateStr + ')';
    if (info.refreshing) {
        text += ' - ' + I18n.t('settings.refreshing', 'Refreshing...');
    }
