...
def _generate_silence_mp3(duration_ms: int) -> bytes:
    ...
    Each MPEG1 Layer3 128kbps 44100Hz frame is 26.12ms and 417 bytes."""
    SILENT_FRAME = b'\xff\xfb\x90\x00' + b'\x00' * 413
    frames_needed = max(1, int(duration_ms / 26.12) + 1)
...
def _freebox_ws_upload(host, session_token, filepath, filename, entry, dl_id):
    ...
    ws = websocket.create_connection(ws_url, timeout=30, header=[f'X-Fbx-App-Auth: {session_token}'])
    try:
        ...
        # Drain pending responses before finalize
        ws.settimeout(0.1)
        try:
            while True:
                ws.recv()
...
def _freebox_transfer(dl_id, filepath, filename, entry):
    ...
    try:
        ...
        try:
            ...
            print(f'[DL] Freebox transfer error (retry): {ex}', file=sys.stderr)
            with vm_download_lock:
                entry['status'] = 'error'
            _cleanup_temp(dl_id, filepath)
    except Exception as ex:
        print(f'[DL] Freebox transfer error: {ex}', file=sys.stderr)
        with vm_download_lock:
            entry['status'] = 'error'
        _cleanup_temp(dl_id, filepath)
