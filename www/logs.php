...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        body { font-family: 'Consolas', 'Monaco', monospace; background: #1a1a2e; color: #eee; margin: 0; padding: 20px; }
        ...
        .btn-truncate { background: #ffa502; color: #000; }
        .btn-truncate.full { background: #a29bfe; }
        .stats { color: #888; font-size: 14px; margin-left: auto; }
        table { width: 100%; border-collapse: collapse; font-size: 13px; }
        th { background: #16213e; padding: 10px; text-align: left; position: sticky; top: 0; z-index: 10; }
...
<body>
    ...
    <div class="controls">
        ...
        <button type="button" class="btn-truncate" id="btn-truncate" onclick="toggleTruncate()">Body: truncate</button>
    ...
    <script>
        ...
        function updateTruncateButton(truncate) {
            var btn = document.getElementById('btn-truncate');
            if (truncate) {
                btn.textContent = 'Body: truncate';
                btn.classList.remove('full');
            } else {
                btn.textContent = 'Body: full';
                btn.classList.add('full');
            }
        }

        function toggleTruncate() {
            fetch('logs-api.php?toggleTruncate=1')
                .then(function(r) { return r.json(); })
                .then(function(config) {
                    updateTruncateButton(config.truncateHttpBody);
                });
        }

        function loadConfig() {
            fetch('logs-api.php?config=1')
                .then(function(r) { return r.json(); })
                .then(function(config) {
                    updateTruncateButton(config.truncateHttpBody);
                });
