...
// Load dependencies and the ProviderAPI class
const fs = require('fs');
const regexCode = fs.readFileSync('./js/regex.js', 'utf8');
eval(regexCode);
// proxyDuidParam is defined in utils.js and used by provider.js
...
describe('ProviderAPI', () => {
    ...
    describe('getUrl', () => {
        it('should return original URL when no proxyUrl is set', () => {
            const api2 = new ProviderAPI('http://example.com', 'user', 'pass');
            expect(api2.getUrl('http://test.com')).toBe('http://test.com');
        });

        it('should return proxied URL when proxyUrl is set', () => {
            const api2 = new ProviderAPI('http://example.com', 'user', 'pass', 'https://proxy.example.com');
            const url = api2.getUrl('http://test.com');
